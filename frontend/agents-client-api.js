...
connectButton.onclick = async () => {
  ...
  const sessionResponse = await fetchWithRetries(`${DID_API.url}/${DID_API.service}/streams`, {
    ...
    body: JSON.stringify({
      source_url: 'https://space-systems-dev-1-forms-1n1k6i-attachmentsbucket-d7j5y9nnvkg8.s3.amazonaws.com/rob.jpeg'
    }),
  });
...
async function agentsAPIworkflow() {
  ...
  const createAgent = await axios.get('/agents/me')
  console.log("Create Agent: ", createAgent.data.agents[1])
  let agentId = createAgent.data.agents[1].id
